...
class LotteryDetail extends React.Component{
    ...
    componentWillMount(){
        let {historyLottery, params} = this.props;

        for (var i = 0; i < historyLottery.length; i ++) {
            let val = historyLottery[i];
            if (val.issue == params.issue) {
                this.setState({
                    mapData: {
                        ...
                        prizePool: val.prizePool || 0,
                        saleTotal: val.saleTotal || 0
                    }
                })
                break;
            }
                        ...
                        <div className="lottery-detail">
                            ...
                            <div className="lottery-sale">
                                ...
                                <div className="sale-price">
                                    <span>{moneyFormat(state.saleTotal)}</span>
                                    <span>{moneyFormat(state.prizePool)}</span>
                                </div>
                            </div>
