...
class Lottery extends React.Component{
    ...
    render(){
        let lotteryList = this.state.lotteryList.map( (val, index) => {
            ...
            } else if (val.lotteryId == '001' || val.lotteryId == '002' || val.lotteryId == '004' || val.lotteryId == '108' || val.lotteryId == '109' || val.lotteryId == '110' || val.lotteryId == '113'){
                return(
                    <li key={val.lotteryId}>
                        <Link className="go-detail" to={`/lotteryhistory/${val.lotteryId}`}>
                            <div className="">
