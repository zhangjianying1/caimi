...
class LotteryHistory extends React.Component{
    ...
    loadPage(p, reset, elem){
        ...
        superagent.get('/orderController/historyIssue').set('Accept', 'application/json').query({lotteryId: params.lotteryId, issue: params.issue, page: p, pageSize: '15'}).then((res) => {
                            ...
                            return (<li key={index}>

                                {
                                   ((val.lotteryId == '001' && val.bonusClass.length > 0) || (val.lotteryId == '113' && val.bonusClass.length > 0)) ?
                                        <Link to={`/lotterydetail/${val.issue}`} className="go-detail">
                                             {lotteryOne(val)}
                                        </Link>
