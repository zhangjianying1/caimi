...
class DorpDown extends React.Component {
    ...
    componentDidMount(){
        if (this.props.isDorp == false) return;
...
//                    this.props.initFN('' , []);
    ...
    render(){
        ...
        return (
            ...
            {this.props.isDorp !==  false ? <div className="up-load"><span ref="upStatus" className={this.state.loading} ></span></div> : null}

                <div ref="main">{this.props.children}</div>
                <ScrollLoad  {...this.props}   reset={() => this.reset()} loading={this.state.loading} />
            </div>
            )
    }
}
DorpDown.propTypes = {
    ...
    urls: React.PropTypes.shape({
        scrollLoad: React.PropTypes.shape({
